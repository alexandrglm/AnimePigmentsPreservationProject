...
As you all know, the *GeGeGe no Kitaro* TV series is currently airing with great acclaim, and colour design for that is handled by Itasaka Yasue-san. While colors for Kitaro, Neko-musume, and many demons are already established in the TV series, do we use those as-is for the theatrical version? Actually, no.

TV series colors are made somewhat brighter for TV screens. TV screens and theatrical film screenings have different colour (or brightness) expression ranges, so theatrical works need screens made with appropriate colors.
...
For any work, I set key characters and create the [basic][foundation] centered on those characters. For Kitaro, colour combinations were already decided, so I adjusted brightness and colour saturation while maintaining that balance. First lowering brightness and saturation somewhat uniformly, then raising sunken colors and restraining excessive colors through trial and error. Just like a balance scale.

The result was somewhat lower saturation and brightness compared to TV colour direction, with added depth to shadow colors. I also restrained the yellowish tones in skin colour compared to the TV series.
...
When I handled colour design for *Ojamajo Doremi* previously, main character colour directions created in the first two years didn't reach 200, showing how many this number is. Moreover, I also create special coloring colour directions for individual scenes, so adding those approaches 400? Or more? This was definitely my theatrical work record for most colour directions (laugh).

This is still just the first part of my work. The real battle is colour direction and inspection of completed coloring. The battle finally enters the front lines.
