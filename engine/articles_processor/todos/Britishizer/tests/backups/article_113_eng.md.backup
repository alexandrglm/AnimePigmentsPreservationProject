...
I think probably all colour designers do this, but when we start creating character colors initially, we first build character colors according to some kind of "basic brightness value for characters" that each designer holds within themselves. Once we've created them all, we place those completed characters on basic backgrounds (art boards) to see (adjust) the balance between backgrounds and cels.
...
This thinking was basic both for the colors I first showed Oda-san that got rejected and for the colors I reorganized based on Oda-san's colour concepts and showed him again.
...
I adjusted the strength of colors themselves so that the character colour balance created when lined up on pure white backgrounds would look the same when placed on high-density backgrounds. I adjusted colors darker and raised saturation levels without breaking the balance between characters.
...
And one more thing. The theatrical promotional poster was drawn and colored by Oda-san based on the finalised colour sample set, but this had subtly different part separations. So I made minor corrections to match the poster too (wry laugh).
...
Based on this completed "basic"—so-called "normal"—colour direction, we then moved into each scene's colors, scene-by-scene colour design.
