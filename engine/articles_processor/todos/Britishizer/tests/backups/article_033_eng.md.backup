...
Hajime always troubles me. He's supposedly the protagonist (laughs), so protagonist-like "presence" and "character standing" are necessary, but he's not a hero show protagonist and supposedly a normal high school student (bitter laugh), so the basics are subdued with one accent colour. So this time too, I considered him after somewhat settling other characters' colors.

Guest characters came up with costume designs close to the original, so I basically referenced the original to balance and place colors. "Opera House" owner Hibiki Shizuka (響静歌) and director Kageshima (影島) were done quite stereotypically. Lake Moon Leona's slightly orange hair colour was Director Ito-kun's preference (laughs). "Vampire Legend" had some characters with gender changed from the original, so those parts were original. Personally, I like Nekoma Junko (猫間純子) (laughs).

So, Hajime. Ultimately, "Opera House" settled on the original book cover's light green-based colour. "Vampire Legend," set entirely in the dim ruined hotel, I considered colors that would show well under the mystery-solving guest room's standing lamp, settling on a red parka with yellow shirt combination.

The old series often had multi-day stories with lots of character costume changes, especially Hajime - when stuck for clothing colour ideas, I'd directly use colors of shirts and jackets I often wore then (laughs), which was quite enjoyable. But this time both works were roughly one-night stories without costume changes - disappointing (laughs).
...
*Kindaichi*'s basics are dialogue drama, so I designed with scenes where characters gather and build conversations as brightness standards for main story. "Opera House" used the fireplace room, "Vampire Legend" used the lounge as colour direction standards. However, these aren't finishing colour direction "normals" but somewhat adjusted for brightness and colour tone matching background spaces. In my case, colour tones under natural light in bright places are basic "normal" colors, and when they enter various spaces, I adjust colour tones considering how they look and what impressions they give.

For example, putting so-called "normal" colored characters directly against incandescent lighting room backgrounds makes them float white against the background and look unnatural. So this time too, I adjusted character colour direction brightness and shadow strength for each scene's space, then added that space's light colour tones. In the Opera House theater darkness, I added background darkness blue for blending.
