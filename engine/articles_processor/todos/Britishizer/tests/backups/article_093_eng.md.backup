...
3. Check colour samples with director and main staff. Decide basic character colors for the work.
4. Create coloring settings (called "colour direction sheets," "colour direction tables," various names) based on decided colors.
5. Based on storyboards, meet with director and art staff about entire content.
6. Create (modify) colour samples matching art boards for each scene like evening, night, etc., deciding scene-by-scene character colors.
7. Also create coloring settings for scene-specific colour direction (special coloring).
...
*Coo: Far From Home* work began moving seriously. Key animation gradually started coming up, in-between work, and cuts finally started going to finishing. Which meant soon we needed to do colour direction and send to coloring, but for that we needed to create something first. Yes, creating settings for coloring work, the blueprint for coloring work - "colour direction sheets."

Creating "colors" and "deciding colors" is important work, but creating these "colour direction sheets" is also an important point in "colour" work.

The cuts we colour direct are painted by finishing coloring staff, but specifying paint numbers for every detail in actual cut artwork is impossible. It's time-consuming and becomes very hard to read. So for characters appearing in many cuts with setting artwork, we create coloring settings called "colour direction sheets" in advance and give them to coloring staff to paint based on those. Creating these is also our job as colour design and colour direction staff.

Even today with digital coloring, we create this work as digital data. Since coloring work is done on computers, colors can be picked from colour direction data with tools and placed directly on screen - that's it. Moving images are completed with exactly the colour direction data colors.

But during cel coloring back then, we instructed this only with paint colour numbers on paper settings, and coloring staff would keep these settings in mind or constantly reference them while applying paint to cels.

How we created such coloring settings "colour direction sheets."
...
We'd write paint numbers on these setting sheets, but depending on the setting, there might be parts not drawn from certain angles, or detail explanations in close-up sizes might be separated into different settings, often split across several sheets. If we made colour direction sheets using original settings as-is, coloring one character would require checking 2-3 colour direction sheets. That would be very inconvenient for colorists.

During coloring work, one hand holds a brush, the other a small paint bottle - both hands are basically always occupied, so flipping through colour direction sheet pages in that situation is very troublesome. Usually they'd post current character colour direction sheets around their coloring desk for reference while painting, but space for posting 2-3 sheets is limited, and needing to check 2-3 sheets is inherently troublesome - too much trouble increases accidents like painting mistakes.
