...
That theater pamphlet uses quite a lot of colour samples and is quite luxurious, and most of those are colour samples that I colored myself during the colour decision process.
...
At the time we were still in the analog era, of course. We painted on cels with paints, finished them, and shot on film. Key animation reaches me → colour direction on key animation → in-between animation & painting work → cels painted in about 5 days → cel inspection & photography → rushes check. Every day was this repetition. That kind of month. In that flow, that it turned out reasonably well was probably because, even with no time, we properly did the decision-making work and got director approval—I think that's the process we followed, but I don't really remember!

Well, most scenes could go with so-called "normal" colors... oh right, come to think of it, there were only a few key scenes where we properly did colour checks—like the rooftop evening scene, the hospital room in the flashback image, stuff like that (sweat).

In today's digital painting era, for example, even if cuts don't have properly finalised colour direction, you can send them to key animation and finishing (仕上げ) anyway, have the painting proceed with "normal" or "dummy colors," then after they're painted, place them on backgrounds to decide colors and repaint the digital painting data before sending to photography (actually, we make things quite routinely in that manner). But in the cel finishing era, naturally such repainting was impossible.
...
Actually, I wasn't entirely unaware of Director Ikuhara's intent, but I was concerned that if I made the cel colors pink-toned in pink light, the characters would blend too much with the light colour and I wouldn't be able to effectively utilize their expressions. So I deliberately finished them with "normal-style" strong contrast colors without shifting toward pink.
