...
**Toei Doga's TV series finishing system at this time assigned one "inspection" member as the person in charge of each work, but usually they only handled program startup and later-appearing "main treatment" character coloring settings (so-called "colour direction sheets"), with per-episode colour direction requested from production companies handling that episode's finishing.**

**As I wrote earlier, basically character colour direction was also handled by art, so guest character colors appearing in each episode were all specified by art director **Kubota-san**, with per-episode finishing production companies communicating with art through art department progress to decide.**

**For example, guest characters appearing in #3 who then appear in several subsequent episodes would get OK from the first episode's direction, then that OK colour direction would be passed to the next episode's production company, and so on.** Usually colors from art were OK as-is, so art's specifications - copies with paint numbers noted on character design drawings - would circulate as-is as colour direction sheets.

**As an aside, due to such systems, most early *Saint Seiya* episode guest characters lack properly organized colour direction sheets.** Toei Doga didn't place "setting production" then or now (assistant directors managed it), so colour direction got quite scattered.

**Also, because of such colour direction sheets, there were many coloring mistakes.** Unlike now where colour designers check all episodes, things other than properly organized main characters were OK if consistent within that episode and production company.

**However, Episode 1 and opening/ending were handled by internal inspection, so Episode 1 was basic - the series standard.** But Episode 1 is the very first, so following direction instructions for colour direction was all I could do. I had no capacity to think about later episodes back then (bitter laugh). Just adding colour direction to in-between animation coming to me one after another.

**Also back then, we weren't allowed to check TV series finishing deliveries.** Delivered cels went directly from finishing progress to **saridashi** (compositing), with assistant directors checking before **saridashi** if time allowed. So colour mistakes and such would only become retakes after being photographed and becoming rush film, or when lined up and connected during editing (well, this system basically hasn't changed even now (bitter laugh)). We often weren't even invited to rushes.
...
**Rewatching reminded me that pupil highlight processing wasn't finalised yet.** Also **Pegasus** **Saint Cloth (聖衣)** highlight processing. How to process highlights on white, like **Marin**'s mask highlights - trial and error (laughs).
...
**"Red, right, ground shadow. X-28, right."** These are **Morishita-san**'s words. About ground shadows under the Saints running toward us in the opening's first cut. This was Morishita-san's first words at the opening colour direction meeting. When paints were used for coloring, we'd communicate colors and colour nuances using paint numbers as common vocabulary like this. So, "X-28, right." Incidentally, blood-like vivid red.
...
**Opening and ending work came after Episode 1 work was finished, and we were pressed for time.** Cels painted at the last minute had colour mistakes, and I have memories of repainting large cels alone at the large-format light table for late-night coloring (laughs).

**Colour mismatches appeared in rushes completed at the last minute, but there was no time to fix them, so they became "lifelong embarrassing retake #1" (bitter laugh).** In an ending cut where large cel hair flowing was on repeat, cel misalignment couldn't be fixed until the end, becoming "lifelong embarrassing retake #2" (bitter laugh).
