...
As I wrote last time, after creating the colors for the regular characters by modifying the existing TV version character colors, and the theatrical original characters, we now move on to matching these with the background art for each scene and confirming the colour scheme for each scene.
...
As for the light effects during battle scenes and reflection effects, those colors were handled almost entirely on a cut-by-cut basis. Oh, and the familiar BANK cuts from the TV version were naturally adjusted in colour to match their use in this theatrical production.

Since the climax scene took place at night, I created night-like colors for the cels, but here too there was an order from the director to "not make it too dark, use colors that still convey the characters' normal colour feel," so I created quite clean night colors.

For daytime scenes inside Kitaro's house and Hana's mansion, I had them colored with the "normal" colour direction first, then made adjustments during cel inspection. I placed the cels on the backgrounds for those cuts and slightly adjusted the brightness.
...
Based on predetermined scene colour directions, we specify colors for each accordingly, but even then, depending on the cut, there are various elements like the amount of shadows in the drawings, the brightness or darkness expression of the backgrounds, the particular performance content we want to achieve in that cut, so it often doesn't work to simply say "this scene's colors are OK if painted with this colour direction."
