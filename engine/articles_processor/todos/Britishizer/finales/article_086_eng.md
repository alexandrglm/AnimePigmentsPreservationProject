...
It broadcasts within the program "Sue Ensa" on NHK Educational every Tuesday at 7:25 PM. Each episode is 5 minutes long, but recklessly it's a continuous story (laughs). I handled the colour design (色彩設計) for the main characters.
...
However, there are cuts that are quite difficult to do in 3D CG, and those follow the conventional animation → coloring process. So it's completed as a mixture of 2D and 3D (therefore, colour direction in the credits refers to colour direction for 2D parts).

My work involves adding colors to hand-drawn character settings from the character designer and deciding on them. Props are basically the same. These are then converted to 3D and made into screens. Naturally there are things where 3D models are created first, and I check the colors on those too, creating unified colour tone. For night and evening scene colors, since 2D and 3D are often mixed, I give instructions and have photography processing bring them together well.
