...
Also, this is direction territory not finishing, but in following scenes, as **Dolbar**'s "technique," expressing drawing opponents into and absorbing them into different dimensions used photography to insert and replace multiple backgrounds. Making wave-pattern masks with animation, having 3-4 different colour scheme backgrounds drawn from the same source drawing, then inserting them during photography. Such techniques are **Yamauchi-san**'s specialty. Ground gouging depictions also used 2 background types for insertion.

Next I tried "night colors." Until then in *Seiya*, night scenes were colored with normal colour direction without particularly changing colors, but I created "night colors" just for the scene where **Seiya** and **Shiryu** run across vast night plains. Not simply making colors one step darker, but adding background spatial colors to dull them down.
...
Well, looking back now, the "night colors" weren't very successful (bitter laugh), but this colour thinking approach would connect to later ***Saint Seiya: Legend of Crimson Youth***.

Besides these "night colors," each scene except image scenes was basically normal colour direction coloring. This was based on considering "character standing" - the policy of "let's keep fighting characters in battle scenes basically normal as before."
